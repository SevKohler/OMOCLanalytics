...
Reads:  archetypesMapped/**/*.adl   Writes: preparedArchetypes/<archetype-id>.md
                                            +  preparedArchetypes/00_INDEX.md
                                            +  preparedArchetypes/archetypes.csv (index as CSV)
...
# Audit / housekeeping elements that record when the item was last edited, not clinical
# content. Excluded from the leaf count the same way reference model attributes are.
AUDIT_ELEMENT_LABELS = {"last updated", "date last updated"}

Element = namedtuple("Element", "code label value_type group")
Slot = namedtuple("Slot", "code label allowed group")
Archetype = namedtuple("Archetype", "id concept category elements slots")

GROUP_SEP = " > "   # joins the internal-cluster ancestry, outermost first
...
def parse_archetype(path, category):
    ...
    elements, slots = [], []
    # Ancestor container stack, one entry per open CLUSTER/HISTORY/EVENT/... node:
    # {indent, internal}. `internal` marks a grouping CLUSTER (not the root, not a slot),
    # whose label disambiguates a leaf that a repeated sub-structure would otherwise flatten.
    stack = []
    root_seen = False
    for i, line in enumerate(block):
        ...
        if not node:
            continue
        indent = len(node.group("indent"))
        code = node.group("code")
        label_match = INLINE_LABEL.search(line)
        label = clean_label(label_match.group(1) if label_match else "")

        # A leaf/container at this indent closes every ancestor at the same or deeper indent.
        while stack and stack[-1]["indent"] >= indent:
            stack.pop()
        group = GROUP_SEP.join(a["label"] for a in stack if a["internal"])

        if node.group("slot"):
            if label.lower() in EXTENSION_LABELS:
                continue
            slots.append(Slot(code, label, slot_allowed_archetypes(block, i), group))
        elif node.group("rm_type") == "ELEMENT":
            if label.lower() in AUDIT_ELEMENT_LABELS:
                continue
            elements.append(Element(code, label, element_value_type(element_body(block, i)), group))
        else:
            # A structural container: keep it on the stack so its descendants know their parent.
            # Only a non-root CLUSTER is an internal grouping cluster (HISTORY/EVENT/ITEM_TREE and
            # the root node carry no disambiguating meaning and are excluded from the group path).
            internal = node.group("rm_type") == "CLUSTER" and root_seen
            stack.append({"indent": indent, "label": label, "internal": internal})
            root_seen = True
...
def render_markdown(arch):
    ...
    lines.append("\n## Element leaves\n")
    if arch.elements:
        if any(e.group for e in arch.elements):
            lines.append("| Code | Within cluster | Element | Value type |")
            lines.append("|------|----------------|---------|------------|")
            lines += [f"| {e.code} | {e.group} | {e.label} | {e.value_type} |" for e in arch.elements]
        else:
    ...
    lines.append("\n## Cluster slot leaves\n")
    if arch.slots:
        if any(s.group for s in arch.slots):
            lines.append("| Code | Within cluster | Slot | Allowed archetypes |")
            lines.append("|------|----------------|------|--------------------|")
            lines += [f"| {s.code} | {s.group} | {s.label} | {s.allowed} |" for s in arch.slots]
        else:
...
def render_index_csv(archetypes):
    """The same index table as render_index, emitted as CSV (one row per archetype).

    Feeds from the identical parsed `archetypes` list, so the numbers can never diverge
    from the markdown index; only the format differs.
    """
    buffer = io.StringIO()
    writer = csv.writer(buffer, lineterminator="\n")
    writer.writerow(["archetype", "category", "concept", "elements", "slots", "leaves"])
    for a in sorted(archetypes):
        writer.writerow([a.id, a.category, a.concept, len(a.elements), len(a.slots),
                         len(a.elements) + len(a.slots)])
    return buffer.getvalue()


def render_leaves_csv(archetypes):
    """Every leaf of every archetype as one CSV row (the per-leaf data layer).

    Columns: archetype, code, kind (element|slot), label, value_type, allowed, group.
    `value_type` is filled for elements, `allowed` for slots. `group` is the internal-cluster
    ancestry of the leaf (empty for a top-level leaf). Same parse as the md files, so the two
    cannot diverge.
    """
    buffer = io.StringIO()
    writer = csv.writer(buffer, lineterminator="\n")
    writer.writerow(["archetype", "code", "kind", "label", "value_type", "allowed", "group"])
    for a in sorted(archetypes):
        for e in a.elements:
            writer.writerow([a.id, e.code, "element", e.label, e.value_type, "", e.group])
        for s in a.slots:
            writer.writerow([a.id, s.code, "slot", s.label, "", s.allowed, s.group])
    return buffer.getvalue()
...
def main():
    ...
    (OUTPUT_DIR / "archetypes.csv").write_text(render_index_csv(archetypes), encoding="utf-8")
    (OUTPUT_DIR / "leaves.csv").write_text(render_leaves_csv(archetypes), encoding="utf-8")

    elements = sum(len(a.elements) for a in archetypes)
    slots = sum(len(a.slots) for a in archetypes)
    print(f"Wrote {len(archetypes)} md files + 00_INDEX.md + archetypes.csv + leaves.csv to {OUTPUT_DIR}")
