...
# Mirror extract_leaves.py: audit/housekeeping elements are excluded from the leaf count.
...
def parse_source(path):
    """Independent parse -> (elements, slots, structural_counter, extension_count, audit_count).

    elements: list of (code, label, value_type);  slots: list of (code, label, allowed).
    audit_count: ELEMENTs dropped as audit/housekeeping (e.g. "Last updated").
    """
    block = definition_block(path.read_text(encoding="utf-8", errors="replace"))
    headers = list(NODE_HEADER.finditer(block))
    elements, slots, structural, extensions, audit = [], [], Counter(), 0, 0
    ...
    return elements, slots, structural, extensions, audit
...
def parse_markdown(path):
    ...
    Header-aware: each table's `| Code | ... |` row names its columns, so an optional
    "Within cluster" disambiguation column is read by name and dropped here. Every element
    row is normalised to (code, label, value_type) and every slot row to (code, label,
    allowed), exactly as before, keeping the source cross-check on the leaf identity.
    """
    ...
    for line in text.splitlines():
        if line.startswith("## Element leaves"):
            section, columns = "element", None
        elif line.startswith("## Cluster slot leaves"):
            section, columns = "slot", None
            ...
            if section == "element":
                elements.append((row.get("Code", ""), row.get("Element", ""), row.get("Value type", "")))
            elif section == "slot":
                slots.append((row.get("Code", ""), row.get("Slot", ""), row.get("Allowed archetypes", "")))
...
def check_archetype(arch_id, source_path, md_path):
    ...
    issues, warnings = [], []
    src_elements, src_slots, _, _, _ = parse_source(source_path)
...
def totals(source_files, ids):
    elements = slots = extensions = audit = 0
    structural = Counter()
    value_types = Counter()
    raw_nodes = 0
    for arch_id in ids:
        src_elements, src_slots, src_structural, ext, aud = parse_source(source_files[arch_id])
    ...
    return elements, slots, extensions, audit, structural, value_types, raw_nodes
...
def main():
    ...
    elements, slots, extensions, audit, structural, value_types, raw_nodes = totals(source_files, shared_ids)
    accounted = elements + slots + extensions + audit + sum(structural.values())
    if raw_nodes != accounted:
        issues.append(f"[conservation] raw nodes {raw_nodes} != elements+slots+extension+audit+structural {accounted}")
    ...
    report = [
        ...
        f"Audit elements excluded (e.g. 'Last updated'): {audit}",
